...
setwd("C:/Users/user/Downloads/R Markdowns")
```
...
plot(table(data$Day), type = "o", main = "Distribution of Visitors across Days", ylab = "No. of visitors", col ="hotpink2")
```
...
plot(table(data$Month), type = "o", main = "Distribution of Visitors across Months", ylab = "No. of visitors", col ="blue")
```
...
#Feature selection
data_f <- data[c('Daily.Time.Spent.on.Site', 'Age', 'Area.Income', 'Daily.Internet.Usage', 'Gender', 'Clicked.on.Ad')]
#Previewing dataset
head(data_f)
```
...
```{r}
intrain <- createDataPartition(y = data_f$Clicked.on.Ad, p= 0.7, list = FALSE)
